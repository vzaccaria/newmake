(function(){
  var _, glob, path, fs, shelljs, debug, targets, phonyTargets, deps, makefile, addToMakefile, addDeps, createTarget, createPhonyTarget, Box, parse, parseWatch, slice$ = [].slice, join$ = [].join;
  ...
  Box = (function(){
    ...
    prototype.createBuild = function(){
      return createTarget(this.buildDir + "", "", "mkdir -p " + this.buildDir);
    };
    ...
    prototype.collect = function(name, array){
      var names, sourceBuildTargets, finfo;
      names = this.unwrapObjects(array);
      sourceBuildTargets = join$.call(names.map(function(it){
        return it.buildTarget;
      }), ' ');
      createPhonyTarget(name, sourceBuildTargets);
      finfo = {};
  ...
  parseWatch = function(b){
    var ref$, log, addChangedFile, Gaze, gaze;
    ...
    return gaze.on('all', function(event, filepath){
      ...
      if (event === 'changed') {
        if (in$(filepath, deps)) {
          ...
          return log("Other event on " + filepath);
        }
      } else {
