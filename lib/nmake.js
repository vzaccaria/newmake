(function(){
  var _, glob, path, fs, shelljs, debug, targets, phonyTargets, deps, cleanTargets, notifyTargets, watchSources, notifyStripString, makefile, resetMakefile, addToMakefile, addDeps, error, Box, parse, watchSourceFiles, watchDestFiles, parseWatch, LIVERELOAD_PORT, lr, startLivereload, notifyChange, moment, color, os, table, name, description, author, year, info, err, warn, src, otm, cwd, setupTemporaryDirectory, removeTemporaryDirectory, usageString, optimist, argv, command, slice$ = [].slice, join$ = [].join;
  ...
  parseWatch = function(b){
    ...
    return shelljs.exec('make all -j', {
      ...
      watchSourceFiles(function(event, filepath){
        ...
        if (event === 'changed') {
          return shelljs.exec('make all -j', function(){});
        } else {
