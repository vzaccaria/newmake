...
(function(){
  var _, glob, path, fs, shelljs, minimatch, debug, hdebug, adebug, targets, phonyTargets, deps, cleanTargets, notifyTargets, watchSources, notifyStripString, notifyRewrites, silent, command, makefile, EXPRESS_PORT, EXPRESS_ROOT, resetMakefile, addToMakefile, addDeps, error, Box, parse, watchSourceFiles, watchDestFiles, parseWatch, startExpress, LIVERELOAD_PORT, lr, startLivereload, notifyChange, moment, color, os, table, name, description, author, year, info, err, warn, src, otm, cwd, setupTemporaryDirectory, removeTemporaryDirectory, usageString, optimist, argv, slice$ = [].slice, join$ = [].join;
  ...
  Box = (function(){
    ...
    function Box(){
      this._prefixName = curry$(prototype.prefixName, true);
      this.prefixName = bind$(this, '_prefixName');
      this._changeFolder = curry$(prototype.changeFolder, true);
      this.changeFolder = bind$(this, '_changeFolder');
      this._changeExtension = curry$(prototype.changeExtension, true);
      this.changeExtension = bind$(this, '_changeExtension');
      ...
      this.serveRoot = bind$(this, 'serveRoot', prototype);
    ...
    prototype.serveRoot = function(root){
      return EXPRESS_ROOT = root;
    };
    ...
    prototype.changeExtension = function(newext, prevFinfo){
      var finfo;
    ...
    prototype.changeFolder = function(folder, prevFinfo){
      var finfo;
      finfo = {
        orig: prevFinfo.prod,
        prod: {}
    ...
    prototype.prefixName = function(prefix, prevFinfo){
      var finfo;
      finfo = {
        orig: prevFinfo.prod,
        prod: {}
    ...
    prototype.compileFiles = function(action, ext, g, localDeps){
      ...
      files = glob.sync(g);
      if (localDeps != null) {
        if (!_.isArray(localDeps)) {
          localDeps = glob.sync(localDeps);
        } else {
          localDeps = _.flatten(_.map(localDeps, function(it){
            return glob.sync(it);
          }));
        }
      } else {
  ...
  startExpress = function(){
    ...
    return console.log("Added connect-livereload: ");
  };
